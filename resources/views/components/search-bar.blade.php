...
<form method="GET" action="{{ route('search') }}" class="w-full">
    <div class="relative">
        ...
        <input
            ...
            class="block w-full pl-10 pr-12 py-3 border border-gray-300 rounded-lg leading-5 bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 sm:text-sm transition"
            required
            minlength="2"
            maxlength="100"
            autofocus
        ...
        <div class="absolute inset-y-0 right-0 pr-3 flex items-center">
            <a
                href="{{ route('home') }}"
                class="text-gray-400 hover:text-gray-500 focus:outline-none focus:text-gray-500 transition"
    ...
    <div class="mt-4 flex flex-wrap gap-2" x-data="{ showFilters: false }">
        ...
        <div x-show="showFilters" x-collapse class="w-full mt-2 p-4 bg-gray-50 rounded-lg space-y-3">
            <!-- Sort Order -->
            <div>
                <label for="order" class="block text-sm font-medium text-gray-700 mb-1">Sort By</label>
                <select
                    name="order"
                    id="order"
                    class="block w-full pl-3 pr-10 py-2 text-base text-gray-900 border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md"
                >
            ...
            <!-- Max Results -->
            <div>
                <label for="maxResults" class="block text-sm font-medium text-gray-700 mb-1">Results Per Page</label>
                <select
                    name="maxResults"
                    id="maxResults"
                    class="block w-full pl-3 pr-10 py-2 text-base text-gray-900 border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md"
